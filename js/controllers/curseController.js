...
module.exports = (db) => ({
    ...
    update: async (req, res) => {
        try { const userId = req.user?.id || req.headers['x-usuario'] || req.headers['x-user-id']; const saved = await service.update(db, req.params.id, req.body, userId); res.json(saved); }
    ...
    remove: async (req, res) => {
        try { const userId = req.user?.id || req.headers['x-usuario'] || req.headers['x-user-id']; const out = await service.remove(db, req.params.id, userId); res.json(out); }
